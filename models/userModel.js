...
export const insertImage = (userId, filePath) => {
  return new Promise((resolve, reject) => {
    db.query(
      "INSERT INTO images (user_id, file_path, created_at, updated_at) VALUES (?, ?, NOW(), NOW())",
      [userId, filePath],
      (err, result) => {
        if (err) return reject(err);
        resolve(result);
...
// Actualizar imagen
export const updateImage = (id, filePath) => {
  return new Promise((resolve, reject) => {
    db.query(
      "UPDATE images SET file_path = ?, updated_at = NOW() WHERE id = ?",
      [filePath, id],
      (err, result) => {
        if (err) return reject(err);
        resolve(result);
