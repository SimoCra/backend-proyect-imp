...
import {
  ...
  updateImage,deleteImage,getImagesByUser
...
export const createImage = async (userId, file) => {
  if (!file) throw "La imagen es requerida";

  const filePath = `uploads/users/${file.filename}`;
  const insertResult = await insertImage(userId, filePath);
...
/**
 * Actualizar la ruta de una imagen existente
 */
export const editImage = async (id, file) => {
  if (!file) throw "La nueva imagen es requerida";

  const filePath = `uploads/users/${file.filename}`;
  await updateImage(id, filePath);
 ...
 */
export const listImagesByUser = async (userId) => {
